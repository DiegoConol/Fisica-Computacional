...
int main(void)
{
    ...
    {
        double Lyapunov = 0.0;
        int pasos_renormalizacion = tiempo; // con h = 0.001 y renormalizando cada paso
        int N = 0;
        ...
        while (t < tiempo)
        {
            ...
            // Calculo la nueva distancia
            double d1 = sqrt(pow(y[0] - l[0], 2) + pow(y[1] - l[1], 2) +
                             pow(vel[0] - vell[0], 2) + pow(vel[1] - vell[1], 2));

            // Acumulo el logaritmo del factor de expansión
            Lyapunov += log(d1 / d0);
            N++;

            // Renormalizo la separación
            double factor = d0 / d1;
            for (int i = 0; i < 4; i++) {
            l[i] = y[i] + (l[i] - y[i]) * (d0 / d1);
            }
        ...
        Lyapunov = Lyapunov / (N * h);  // Promedio temporal

        fprintf(lyapunovtxt, "%lf %d\n", Lyapunov, tiempo);
        printf("Terminé para tiempo %d\n", tiempo);
        fflush(stdout);
    }
