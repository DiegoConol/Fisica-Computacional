...
#define T_Total 50      //tiempo total de simulacion

#define incremento 50   //Incremento de tiempo
#define Tmaximo  500  //Tiempo máximo para lyapunov.

#define E 12.5 //Energía total de cada péndulo.
...
while(t<tiempo)
{
    ...
    // Sumo la diferencia actual a la diferencia total
    diferencia_total += sqrt(pow(y[0]-l[0], 2)+ pow(y[1]-l[1], 2) + pow(vel[0]-vell[0], 2) + pow(vel[1]-vell[1], 2));

    t=t+h;
}

// Calculo el Lyapunov como la suma total
double Lyapunov = diferencia_total;
