...
#define h 0.006              //Paso temporal
#define PI 3.14159265       //Pi
#define T_TOTAL 700       //Tiempo total de simulación
#define mod 0.0 //Modulo de la velocidad
#define mod2 1.1 //Calentar el sistema

#define particula 5 //Partícula a la que le voy a calcular la fluctuación de posición.
#define numpasosmedio 20 //Esto se usa para hacer la fluctuación media cada numpasosmedio pasos temporales.
...
int main(void)
{
    ...
    //EL APARTADO 7 SE BASA EN AUMENTAR LA TEMPERATURA MUY POCO A POCO, ASI QUE TRAS LAS MARCAS REESCALAMOS LA VELOCIDAD.
    //El factor de reescala es 1.1

    //AL PASAR 160 SEGUNDOS, LA VELOCIDAD SE REESCALA CADA 20 SEGUNDOS, HASTA 240 SEGUNDOS.

    int tiempo= (int) numpasos/T_TOTAL; //Número de pasos temporales por segundo

    int t_60 = 60*tiempo; //Número de pasos temporales para 60 segundos
    int t_60_ok = 0; //Variable booleana para controlar si se ha llegado a 60 segundos

    int t_120 = 120*tiempo; 
    int t_120_ok = 0; 

    int t_160 = 160*tiempo;
    int t_160_ok = 0;

    int t_180 = 180*tiempo; 
    int t_180_ok = 0; 

    int t_200 = 200*tiempo;
    int t_200_ok = 0;

    int t_220 = 220*tiempo;
    int t_220_ok = 0; 
    
    int t_240 = 240*tiempo;
    int t_240_ok = 0; 

    int t_300 = 300*tiempo;
    int t_300_ok = 0;

    int t_360 = 360*tiempo;
    int t_360_ok = 0;

    int t_420 = 420*tiempo;
    int t_420_ok = 0;

    int t_480 = 480*tiempo;
    int t_480_ok = 0;

    int t_540 = 540*tiempo;
    int t_540_ok = 0;

    int t_600 = 600*tiempo;
    int t_600_ok = 0;

   double fluc[N][2]; //Fluctuación de la partícula elegida
   double fluct[N];

   double flucmedia [N]; //Fluctuación media de la partícula elegida

   for (int i=0; i<N; i++)
   {
       fluc[i][0] = 0.0; //Fluctuación en x
       fluc[i][1] = 0.0; //Fluctuación en y
       flucmedia[i] = 0.0; //Fluctuación media en x e y
       fluct[i] = 0.0; //Fluctuación total
   }
    ...
    for (int t=0; t<numpasos; t++)
    {
        ...
        //Ahora hago la fluctuación media de un numero de pasos concreto
        for (int i=0; i<N; i++)
        {
            fluc[i][0] = dr[i][particula][0]; //Fluctuación en x
            fluc[i][1] = dr[i][particula][1]; //Fluctuación en y
            fluct[i] = fluc[i][0]*fluc[i][0]+fluc[i][1]*fluc[i][1]; //Fluctuación total
            flucmedia[i] += fluct[i]; //Suma de fluctuaciones
            fluct[i] = 0.0; //Reinicio la fluctuación total para el siguiente paso
        ...
        for (int i=0; i<N; i++)
        {
         ...
         for (int i=0; i<N; i++)
         {
             f[i][0] = -r[i][0]+r[particula][0]; //Fluctuación en x
             f[i][1] = -r[i][1]+r[particula][1]; //Fluctuación en y
        ...
        if (t == t_60 && t_60_ok == 0 ) //Si ha pasado 20 segundos
        {
            for (int i=0; i<N; i++)
            {
                v[i][0] *= mod2; 
            ...
            t_60_ok = 1; //Cambio la variable booleana a 1 para que no vuelva a entrar aquí.
        }
        if (t == t_120 && t_120_ok == 0 ) 
        {
            for (int i=0; i<N; i++)
            {
                v[i][0] *= mod2; 
